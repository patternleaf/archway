...
// Adapted from localstorage_adapter.js
...
(function () {
	...
	DS.CSSerializer = DS.JSONSerializer.extend({
	...
	DS.CSAdapter = DS.Adapter.extend(Ember.Evented, {
		...
		*/
		find: function(store, type, id, opts) {
			...
			var _this = this;
			_debug('CSAdapter.find', arguments);
			return new Ember.RSVP.Promise(function(resolve, reject) {
				_this._namespaceForType(type).then(function(namespaceRecords) {
					var record = Ember.A(namespaceRecords.records[id]);
					/**
					...
					if (!record || !record.hasOwnProperty('id')) {
						reject(
							new Error("Couldn't find record of" + " type '" + type.typeKey + "' for the id '" + id + "'.")
						);
					}

					if (allowRecursive) {
						resolve(_this.loadRelationships(type, record));
					} else {
						resolve(record);
					}
				}, function(error) {
		...
		findMany: function (store, type, ids, opts) {
			...
			_debug('CSAdapter.findMany', arguments);
			return new Ember.RSVP.Promise(function(resolve, reject) {
				adapter._namespaceForType(type).then(function(namespaceRecords) {
					...
					for (var i = 0; i < ids.length; i++) {
						record = namespaceRecords.records[ids[i]];
						if (!record || !record.hasOwnProperty('id')) {
							reject(
								new Error("Couldn't find record of type '" + type.typeKey + "' for the id '" + ids[i] + "'.")
							);
						}
		...
		findQuery: function (store, type, query, recordArray) {
			var _this = this;
			_debug('CSAdapter.findQuery', arguments);
			return new Ember.RSVP.Promise(function(resolve, reject) {
				_this._namespaceForType(type).then(function(namespaceRecords) {
					var results = _this.query(namespaceRecords.records, query);
					if (results.get('length')) {
						resolve(this.loadRelationshipsForMany(type, results));
					} else {
						reject();
					}
				}, function(error) {
		...
		query: function (records, query) {
			var results = [], record;
			_debug('CSAdapter.query', arguments);
		...
		findAll: function (store, type) {
			var _this = this;
			_debug('CSAdapter.findAll', arguments);
			return new Ember.RSVP.Promise(function(resolve, reject) {
				_this._namespaceForType(type).then(function(namespaceRecords) {
					_debug('findAll _namespaceForType(' + type + ') returned ', namespaceRecords);
					var results = [];
					for (var id in namespaceRecords.records) {
						results.push(Ember.copy(namespaceRecords.records[id]));
					}
					resolve(results);
				}, function(error) {
					reject(error);
		...
		createRecord: function (store, type, record) {
			var _this = this;
			_debug('createRecord for record', record);
			var recordHash = record.serialize({ includeId: true });
			return new Ember.RSVP.Promise(function(resolve, reject) {
				_debug('calling _namespaceForType, type ', type);
				_this._namespaceForType(type).then(function(namespaceRecords) {
					_debug('createRecord setting records', recordHash.id, recordHash);
					namespaceRecords.records[recordHash.id] = recordHash;
					_this.persistData(type, namespaceRecords).then(function(data) {
						resolve(data);
					}, function(error) {
						reject(error);
					});
				}, function(error) {
					_debug('in createRecord, namespaceForType error: ', error);
					reject(error);
				});
			});
			/*
			...
			return this.persistData(type, namespaceRecords);
			*/
		},
		...
		updateRecord: function (store, type, record) {
			...
			return new Ember.RSVP.Promise(function(resolve, reject) {
				_this._namespaceForType(type).then(function(namespaceRecords) {
					_debug('updateRecord setting record with id', id, 'to', record.serialize({ includeId: true }));
		...
		adapterNamespace: function () {
			return this.get('namespace') || 'DS.CSAdapter';
		},

		// don't write to chrome storage on every update. coalesce
		// updates and write when the current call stack/promise chain
		// has collapsed.
		//
		// equivalent to the "archway" local storage location.
		tempStorage: { },
		
		getStorage: function () {
			var _this = this;
			var namespace = _this.adapterNamespace();
			var result = null;
			if (!_(this.tempStorage).isEmpty()) {
				return new Ember.RSVP.Promise(function(resolve, reject) {
					resolve(_this.tempStorage);
				});
			}
			else {
				return new Ember.RSVP.Promise(function(resolve, reject) {
					//_debug('fetching from localstorage key', namespace);
					chrome.storage.local.get(namespace, function(storageResult) {
						if (!chrome.runtime.lastError) {
							_debug('storage.get returned ', storageResult);
							if (namespace in storageResult) {
								_this.tempStorage = storageResult[namespace];
							}
							else {
								_this.tempStorage = { };
							}
		...
		persistData: function(type, data) {
			...
			_debug('persistData for', type);
			return new Ember.RSVP.Promise(function(resolve, reject) {
				_this.getStorage().then(function(storage) {
					
					storage[modelNamespace] = data;
					
					if (0/*_this.pendingPersists == 1*/) {
						// last persist. do the thing.
						_debug('persistData actually calling storage.set with data ', storage);
						var storageHash = {};
						storageHash[_this.adapterNamespace()] = storage;
						chrome.storage.local.set(storageHash, function() {
							if (chrome.runtime.lastError) {
						...
						_debug('persistData wrote', type, data, ' to temp store', storage);
						// writing to our temp store was the persist.
						if (!_this.localWriteScheduled) {
							Ember.run.next(_this, _this.writeToLocalStore);
							_this.localWriteScheduled = true;
						}
						resolve();

					}
					_this.pendingPersists--;
				});
			});
			
			
			/*
			if (this.pendingPersists > 0) {
				_debug('persistData: pending persists: ' + this.pendingPersists + ', ');
				return new Ember.RSVP.Promise(function(resolve, reject) {
					_this.tempStorage[modelNamespace] = data;
					if (_this.pendingPersists == 1) {
						_debug('persistData actually calling storage.set');
						var storageHash = {};
						storageHash[_this.adapterNamespace()] = JSON.stringify(_this.tempStorage);
						chrome.storage.local.set(storageHash, function() {
							if (chrome.runtime.lastError) {
								reject(chrome.runtime.lastError);
							}
				...
				_debug('no pending persists, type ' + type + ', starting with a getStorage()');
				return new Ember.RSVP.Promise(function(resolve, reject) {
					_this.getStorage().then(function(localStorageData) {
						if (_(_this.tempStorage).isEmpty()) {
							_this.tempStorage = localStorageData;
						}
						_this.tempStorage[modelNamespace] = data;
						_this.pendingPersists--;
						resolve();
			...
			*/
			...
			return new Ember.RSVP.Promise(function(resolve, reject) {
				_this.getStorage().then(function(localStorageData) {
					console.log('data before: ', JSON.stringify(localStorageData));
					console.log('updating property ', modelNamespace);
					localStorageData[modelNamespace] = data;
					console.log('after: ', JSON.stringify(localStorageData));
					var storageHash = {};
					var json = JSON.stringify(localStorageData);
					storageHash[_this.adapterNamespace()] = json;
		...
		localWriteScheduled: false,
		writeToLocalStore: function() {
			var _this = this;
			// make sure we have a snapshot of the store at calltime.
			var storage = Ember.copy(this.tempStorage, true);
			this.localWriteScheduled = false;
			return new Ember.RSVP.Promise(function(resolve, reject) {
				var storageHash = {};
				storageHash[_this.adapterNamespace()] = storage;
				console.log('calling storage.local.set', storageHash);
				chrome.storage.local.set(storageHash, function() {
					if (chrome.runtime.lastError) {
						console.log('could not write to local store ', chrome.runtime.lastError);
						reject(chrome.runtime.lastError);
					}
					else {
		...
		loadFromJSON: function(json) {
			var _this = this;
			return new Ember.RSVP.Promise(function(resolve, reject) {
				console.log('trying to load from json ', json);
				try {
					_this.tempStorage = JSON.parse(json);
				}
				catch (parseError) {
					reject(parseError);
				}
		...
		/*
		fileWriter: null,
		writeToSandboxedFile: function(json) {
			var _this = this;
			var data = new Blob([json], { type: 'text/json' });
			if (!this.fileWriter) {
				window.requestFileSystem(window.TEMPORARY, 1024*1024, function(fs) {
					fs.root.getFile('scene.json', { create: true }, function(fileEntry) {
						fileEntry.createWriter(function(fileWriter) {
							...
							fileWriter.onerror = function(event) {
								console.log('write error: ', event.toString());
							}
							fileWriter.write(data);
						});
					});
				}, function() {
					console.error('error requestFileSystem', arguments);
				});
			}
			else {
